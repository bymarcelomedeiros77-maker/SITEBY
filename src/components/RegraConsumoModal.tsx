...
export const RegraConsumoModal: React.FC<RegraConsumoModalProps> = ({
    ...
    const { tamanhos, addRegraConsumo, updateRegraConsumo, addToast } = useApp();
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!referencia || consumoUnitario <= 0) {
            addToast('error', "Preencha a referência e o consumo unitário.");
            return;
        }
        ...
        try {
            ...
            console.error(error);
            addToast('error', "Erro ao salvar regra.");
        } finally {
            setIsLoading(false);
        }
    };
