...
import {
    ...
    BarChart3, ShoppingBag, Clock, Globe
} from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import * as XLSX from 'xlsx';
import { MapaBrasil } from '../components/MapaBrasil';
import { ClienteMapaDashboard } from '../components/ClienteMapaDashboard';
import { CnpjTab } from '../components/CnpjTab';
...
export const Clients = () => {
    ...
    const [activeTab, setActiveTab] = useState<'details' | 'cnpj' | 'address' | 'history' | 'notes'>('details');
    ...
    // Vista principal: lista ou mapa ou cnpj
    const [mainView, setMainView] = useState<'lista' | 'mapa' | 'cnpj'>('lista');
    const [mapFiltroCategoria, setMapFiltroCategoria] = useState<ClienteCategoria | 'ALL'>('ALL');
        ...
        <div className="flex flex-col h-full gap-6">
            ...
            <div className="flex items-center gap-1 bg-slate-900/60 border border-slate-800 rounded-xl p-1 self-start">
                <button
                    ...
                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold uppercase tracking-wider transition-all ${mainView === 'lista' ? 'bg-violet-600/80 text-white shadow' : 'text-slate-500 hover:text-white'}`}
                >
                    <Users size={13} /> Lista
                </button>
                <button
                    onClick={() => setMainView('mapa')}
                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold uppercase tracking-wider transition-all ${mainView === 'mapa' ? 'bg-violet-600/80 text-white shadow' : 'text-slate-500 hover:text-white'}`}
                >
                    <Globe size={13} /> Mapa
                </button>
                <button
                    onClick={() => setMainView('cnpj')}
                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold uppercase tracking-wider transition-all ${mainView === 'cnpj' ? 'bg-violet-600/80 text-white shadow' : 'text-slate-500 hover:text-white'}`}
                >
                    <Building2 size={13} /> Consulta CNPJ
                </button>
            </div>

            {/* Conteúdo principal: Mapa, Lista ou CNPJ */}
            {mainView === 'cnpj' ? (
                <div className="flex-1 bg-slate-900/40 border border-slate-800 rounded-2xl p-8 overflow-y-auto custom-scrollbar shadow-2xl">
                ...
                <div
                    ...
                    style={{
                        background: 'radial-gradient(ellipse at center, rgba(10,5,30,0.98) 0%, rgba(5,5,15,1) 100%)',
                ...
                >
                    <div className="flex-1 relative">
                        <MapaBrasil
                            clientes={clientes}
                            filtroCategoria={mapFiltroCategoria}
                            onFiltroChange={setMapFiltroCategoria}
                        />
                    </div>
                    <div
                        className="w-64 flex-shrink-0 p-3 overflow-y-auto"
                        style={{ borderLeft: '1px solid rgba(139,92,246,0.15)' }}
                    >
                        <ClienteMapaDashboard
                            clientes={clientes}
                            filtroCategoria={mapFiltroCategoria}
                            onFiltroChange={setMapFiltroCategoria}
                        />
